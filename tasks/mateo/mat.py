...
def equal(A, B):
    ...
    for coord in A.f:
        if A[coord] != 0 and coord not in B.f:
            return False
        elif coord in B.f and A[coord] != B[coord]:
            return False
    for coord in B.f:
        if B[coord] != 0 and coord not in A.f:
            return False
        # elif B[coord] != 0 and B[coord] != A[coord]:
        #     return False
    return True
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(coord[1], coord[0]): M[coord] for coord in M.f})
...
def vector_matrix_mul(v, M):
    ...
    ret = Vec(M.D[1], {})
    for c in M.D[1]:
        ret[c] = v * Vec(M.D[0], {m: M[m, c] for m, n in M.f if n == c})
    return ret
...
def matrix_matrix_mul(A, B):
    ...
    for r in A.D[0]:
        for c in B.D[1]:
            ret[r, c] = Vec(A.D[1], {col: A[_, col] for _, col in A.f if _ == r }) * Vec(B.D[0], {row: B[row, _] for row, _ in B.f if _ == c})
    return ret
################################################################################
